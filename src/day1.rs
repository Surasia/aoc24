...
pub fn day1_p1() {
    let mut file = File::open(PATH_TO_INPUT).unwrap();
    ...
    for line in br.lines().map_while(Result::ok) {
        ...
        let (left, right) = line.split_once(WHITESPACE).unwrap();
        left_nums.push(left.parse::<u32>().unwrap());
        right_nums.push(right.parse::<u32>().unwrap());
    }
...
pub fn day1_p2() {
    ...
    for num in left_nums {
        ...
        let similarity_score = u32::try_from(occurences.count()).unwrap() * num;
